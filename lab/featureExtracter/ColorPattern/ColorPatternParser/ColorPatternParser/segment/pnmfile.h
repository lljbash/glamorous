...
static image<rgb> *Ipl2Image(IplImage * img) {
	assert(img != NULL);
	...
	for (int i=0;i<img->height;i++)
	{
		...
		{
			uchar* ptr=(uchar *)imPtr(im, j, i);
			ptr[0]=CV_IMAGE_ELEM(img,BYTE,i,j*3+2);
			ptr[1]=CV_IMAGE_ELEM(img,BYTE,i,j*3+1);
			ptr[2]=CV_IMAGE_ELEM(img,BYTE,i,j*3);
		}
	}
	return im;
}

static IplImage* SaveCVimg(image<rgb> *im, const char *name) {
	int width = im->width();
	int height = im->height();
	...
	for (int i=0;i<img->height;i++)
	{
		...
		{
			uchar* ptr=(uchar *)imPtr(im, j, i);
			CV_IMAGE_ELEM(img,BYTE,i,j*3+2)=ptr[0];
			CV_IMAGE_ELEM(img,BYTE,i,j*3+1)=ptr[1];
			CV_IMAGE_ELEM(img,BYTE,i,j*3)=ptr[2];
		}
	}
