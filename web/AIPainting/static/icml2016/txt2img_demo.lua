function wrapper( net_gen, net_txt, queries, dataset, output_dir )
  opt = {
    filenames = '',
    dataset = 'cub',
...
function txt2img( opt )
  ...
  for i = 1,#fea_txt do
    ...
    local visdir = string.format('%s/%s', opt.output_dir, opt.dataset)
    lfs.mkdir(string.format('%s', opt.output_dir))
    lfs.mkdir(visdir)
